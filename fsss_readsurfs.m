function surfs  = fsss_readsurfs (subid, subjects_dir, cfg)
% reads various Freesurfer surfaces, annotations, sub-surfaces for 
%   particular regions.
%
% [USAGE]
...
% .surf.(anything like surf/?h.*)
%
% - read surface morphometrics
% .meas.thickness
% .meas.sulc
% .meas.curv
% .meas.area
% .meas.(anything like surf/?h.*)
%
% - read annotations
% .annot.aparc2005
% .annot.aparc2009
% .annot.(anything like label/?h.*.annot)
% 
% - create isocontours
% .contour.isocurv
%
...
for s=1:2
  hemi=hemi{s};
  fname = fullfile(subjects_dir,subid,'surf',[hemi,'.pial']);
  if cfg.ras
  ...
  fname = fullfile(subjects_dir,subid,'surf',[hemi,'.white']);
  if cfg.ras
  ...
  fname = fullfile(subjects_dir,subid,'surf',[hemi,'.sphere']);
  ...
  thns  = read_curv(fullfile(subjects_dir,subid,'surf',[hemi,'.thickness']));
  ...
  fname = fullfile(subjects_dir,subid,'surf',[hemi,'.inflated']);
  if cfg.ras
    [verts3, faces] = FS_read_surf_ras(fname);
  else
  ...
  if cfg.smoothwm
    fname = fullfile(subjects_dir,subid,'surf', ...
      [hemi,'.smoothwm.n',num2str(cfg.smoothwm)]);
    if strcmp(subid,'fsaverage')
      meanEdgeLength = 0.7481;
    elseif strcmp(subid,'fsaverage6')
      meanEdgeLength = 1.5134;
    elseif strcmp(subid,'fsaverage5')
    ...
    if ~exist(fname,'file') %|| 1
      fname1 = fullfile(subjects_dir,subid,'surf',[hemi,'.white']);
      copyfile(fullfile(subjects_dir,subid,'surf',[hemi,'.curv']),...
        fullfile(subjects_dir,subid,'surf',[hemi,'.curv.old']));
      copyfile(fullfile(subjects_dir,subid,'surf',[hemi,'.area']),...
        fullfile(subjects_dir,subid,'surf',[hemi,'.area.old']));
      ...
      copyfile(fullfile(subjects_dir,subid,'surf',[hemi,'.curv.old']),...
        fullfile(subjects_dir,subid,'surf',[hemi,'.curv']));
      copyfile(fullfile(subjects_dir,subid,'surf',[hemi,'.area.old']),...
        fullfile(subjects_dir,subid,'surf',[hemi,'.area']));
    end
    if cfg.ras
  ...
  if cfg.smoothpial
    ...
    if ~exist(fname,'file')
      fname1 = fullfile(subjects_dir,subid,'surf',[hemi,'.pial']);
  ...
  fname = fullfile(subjects_dir,subid,'label',[hemi,'.aparc.annot']);
  ...
  fname = fullfile(subjects_dir,subid,'label',[hemi,'.aparc.a2009s.annot']);
  ...
  fname = fullfile(subjects_dir,subid,'surf',[hemi,'.curv']);
  whitecurv = read_curv(fname);
  WHITECURV{s} = whitecurv;
  
  fname = fullfile(subjects_dir,subid,'surf',[hemi,'.curv.pial']);
  if ~exist(fname,'file')
  ...
  % read sulcal depth
  fname = fullfile(subjects_dir,subid,'surf',[hemi,'.sulc']);
  ...
  if cfg.suptemp
    ...
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.suptemp.patch']);
  ...
  if cfg.suptemp2
    ...
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.suptemp2.patch']);
  ...
  if cfg.suptemp3
    ...
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.suptemp3.patch']);
  ...
  if cfg.suptemp4
    ...
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.suptemp4.patch']);
  ...
  if cfg.hn
    ...
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.HN.patch']);
    patch  = read_patch(fname1);
    % in some cases, the # of vertices are different due to the flattening
  ...
  if cfg.hnn
    ...
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.HNN.patch']);
    patch  = read_patch(fname1);
    % in some cases, the # of vertices are different due to the flattening
  ...
  if cfg.flat
    ...
    fname2= fullfile(subjects_dir, subid, 'surf',[hemi{s},'.suptemp.flat.patch']);
  ...
  if cfg.flat2
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.suptemp2.patch']);
    % find (or create) a flattened patch
    fname2= fullfile(subjects_dir, subid, 'surf',[hemi{s},'.suptemp2.flat.patch']);
    if ~exist (fname2, 'file')
      system(['mris_flatten ',fname1,' ',fname2]);
    end
    flatpatch = read_patch(fname2);
  ...
  if cfg.hn
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.HN.patch']);
    % find (or create) a flattened patch
    fname2= fullfile(subjects_dir, subid, 'surf',[hemi{s},'.HN.flat.patch']);
    if ~exist (fname2, 'file')
      system(['mris_flatten ',fname1,' ',fname2]);
    end
    flatpatch = read_patch(fname2);
  ...
  if cfg.hnn
    fname1 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.HNN.patch']);
    % find (or create) a flattened patch
    fname2 = fullfile(subjects_dir, subid, 'surf',[hemi{s},'.HNN.flat.patch']);
    if ~exist (fname2, 'file')
      system(['mris_flatten ',fname1,' ',fname2]);
    end
    flatpatch    = read_patch(fname2);
  ...
  if cfg.area
    AREA{s} = read_curv([subjects_dir,'/',subid,'/surf/',hemi{s},'.area']);
  end
end
